    ...
    <div class="flex">
      <div class="w-100 md:w-1/4 pr-2 pb-2">
        ...
        <div class="flex flex-col mb-4 ">
          <label>Nombre</label>
          <input type="text" class="input input-bordered w-full " v-model="currentStaff.title">
        </div>
        <div class="flex flex-col mb-4">
          <label>Color del fondo</label>
          <input type="color" class="input input-bordered w-full " v-model="currentStaff.backgroundColor">
        </div>
        <div class="flex flex-col mb-4">
          <label>Color del texto y del borde</label>
          <input type="color" class="input input-bordered w-full " v-model="currentStaff.textColor">
        </div>
        ...
        <div class="card border border-primary p-4 shadow-lg w-full">
          ...
          <div class="collapse bg-base-200 w-full md:w-1/2 mb-4">
            ...
            <div class="collapse-title text-xl font-medium">
              <span class="mr-2">{{ newRecurringShiftVisible ? '-' : '+' }}</span> Crea un nuevo turno frecuente
            </div>
            <div class="collapse-content"> 
              <div class="w-100 md:w-1/3 mb-4">
                <div class="flex flex-col mb-4 ">
                <label>Hora de inicio</label>
                <input type="time" class="input input-bordered w-full " v-model="tempShift.startTime">
              </div>
              <div class="flex flex-col mb-4 ">
                <label>Hora de fin</label>
                <input type="time" class="input input-bordered w-full " v-model="tempShift.endTime">
              </div>
              </div>
              <h4 class="font-bold text-lg mb-2">Selecciona los dias en que se repite el turno</h4>
              <div class="w-100 md:w-1/3 mb-4">
                <label class="label cursor-pointer">
                  <span class="label-text">Lunes</span> 
                  <input type="checkbox" checked="checked" class="checkbox"  v-model="mapDaysOfWeek[1].checked"/>
                </label>
                <label class="label cursor-pointer">
                  <span class="label-text">Martes</span> 
                  <input type="checkbox" checked="checked" class="checkbox" v-model="mapDaysOfWeek[2].checked"/>
                </label>
                <label class="label cursor-pointer">
                  <span class="label-text">Miercoles</span> 
                  <input type="checkbox" checked="checked" class="checkbox" v-model="mapDaysOfWeek[3].checked" />
                </label>
                <label class="label cursor-pointer">
                  <span class="label-text">Jueves</span> 
                  <input type="checkbox" checked="checked" class="checkbox"  v-model="mapDaysOfWeek[4].checked"/>
                </label>
                <label class="label cursor-pointer">
                  <span class="label-text">Viernes</span> 
                  <input type="checkbox" checked="checked" class="checkbox"  v-model="mapDaysOfWeek[5].checked"/>
                </label>
                <label class="label cursor-pointer">
                  <span class="label-text">Sabado</span> 
                  <input type="checkbox" checked="checked" class="checkbox"  v-model="mapDaysOfWeek[6].checked"/>
                </label>
                <label class="label cursor-pointer">
                  <span class="label-text">Domingo</span> 
                  <input type="checkbox" checked="checked" class="checkbox"  v-model="mapDaysOfWeek[0].checked"/>
                </label>
              </div>
              <div class="w-full  flex justify-end">
                <button @click="saveRecurrentShift" class="btn btn-primary">Guardar nuevo turno frecuente</button>
              </div>
            </div>
          </div>
          ...
          <div class="collapse bg-base-200 w-full ">
            ...
            <div class="collapse-title text-xl font-medium">
              <span class="mr-2">{{ shiftListVisible ? '-' : '+' }}</span> Turnos frecuentes
            </div>
            <div class="collapse-content">
              <Loading v-if="loadingGetRecurrentShifts" />
              <div v-else>
                <div v-for="shift in recurrentShifts" :key="shift.id"  class="flex justify-between items-center border border-primary bg-white rounded px-4 py-2 mb-4 ">
                  
                  <div class="flex justofy-start items-center">
                    <input @change="saveRecurrentShift(shift)" type="checkbox" checked="checked" class="checkbox mr-4" v-model="shift.active" />
                  <div class="mr-4">
                    <div class="flex">
                      <span 
                    ...
                    <div>
                      {{ formatTime({timeString:shift.startTime}) }} - {{ formatTime({timeString:shift.endTime}) }}
                    </div>
                  </div>
                  </div>
                  <button @click="deleteRecurrentShift(shift)" class="align-self-end btn btn-outline btn-error btn-xs">Elimina</button>
                </div>
              </div>
            </div>
          </div>
...
import { putSupaData, postSupaData, getSupaDataWithFilter, deleteSupaData } from '@/utils/supabaseApiService'
...
function deleteRecurrentShift(shift){
  deleteSupaData({tableName:'recurrent-shift', item:shift})
    .then(res =>{
      if(res){
        getRecurrentShiftsForCurrentStaff()
      }
...
function getRecurrentShiftsForCurrentStaff(){
  ...
  getSupaDataWithFilter({tableName:'recurrent-shift', filterColumn:'staff_id', filterValue:route.params.idStaff})
    .then(res =>{
